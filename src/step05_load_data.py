...
# -*- coding: utf-8 -*-
"""
Step 05 — Cargador de datos
Carga intermedio_bd.xlsx en MySQL usando INSERT ... ON DUPLICATE KEY UPDATE.
"""
...
from config.settings import INTERMEDIO_BD, DB_CFG
...
def load_intermediate_excel(path):
    ...
    sheets = {}
    for key, name in [
        ("genes", SHEET_GENES), ("fenotipos", SHEET_FENOS), ("vias", SHEET_VIAS),
        ("articulos", SHEET_ART), ("variantes", SHEET_VARS), ("asociaciones", SHEET_ASSOC),
    ]:
        try:
            sheets[key] = get_sheet(name)
        except Exception as exc:
            logger.warning("No se pudo leer la hoja '%s': %s", name, exc)
...
# ===================== PIPELINE =====================
def main():
    logger.info("Cargando datos desde: %s", INTERMEDIO_BD)
    data = load_intermediate_excel(INTERMEDIO_BD)
    ...
    try:
        ...
        # 3) VIAS: construye mapa por nombre y por reactome_id
        via_to_id = {}
        for _, r in df_vias.iterrows():
            vname = r.get("nombre")
            rid   = r.get("reactome_id")      # columna correcta (antes: reactome_stid)
        ...
        for _, r in df_assoc.iterrows():
            ...
            # 1º: intentar por reactome_id
            rid_as = r.get("reactome_id")     # columna correcta (antes: reactome_stid)
            if rid_as:
        ...
        cnx.commit()
        logger.info("Paso 5 completado: carga confirmada (COMMIT).")

    except Exception as e:
        cnx.rollback()
        logger.error("Error en la carga, se hizo ROLLBACK: %s", repr(e))
        raise
    finally:
        cur.close()
        cnx.close()
